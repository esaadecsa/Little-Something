...
(function () {
  ...
  const STORE_KEY = "littleSomethingLivingStoryV3";
  ...
  function getMessage(info, state) {
    ...
    if (special) {
      return {
        ...
        day:
          special.day ||
          `DAY ${String(info.cycleDay).padStart(2, "0")} · something a little different`,
        title: fillName(special.title),
        text: fillName(special.text),
        button: special.button || "Aku baca. ✦",
        birthday: false,
        special: true,
    ...
    if (returning) {
      return {
        ...
        day: `DAY ${String(info.cycleDay).padStart(2, "0")} · ${monthWords}`,
  ...
  function markShown(info, state, message) {
  ...
  // Pastikan pesan hari ini konsisten walau modal dibuka berkali-kali
  // di tanggal yang sama (mis. buka-tutup app beberapa kali sehari).
  // Pesan baru cuma dihitung ulang saat tanggalnya (WIB) berbeda dari
  // kunjungan terakhir yang tercatat.
  function getOrBuildMessage(info, state) {
    const isNewDay = state.lastShownDate !== info.key;

    if (!isNewDay && state.lastMessage) {
      return state.lastMessage;
    }

    // getMessage() perlu state SEBELUM ditandai, supaya deteksi
    // "welcome back" (gap hari) masih akurat.
    const message = getMessage(info, state);
    markShown(info, state, message);
    return message;
  }
  ...
  function dismissModal() {
    ...
    const afterClose = window.__livingStoryAfterClose;
    window.__livingStoryAfterClose = null;
    if (typeof afterClose === "function") {
      window.setTimeout(() => {
        try { afterClose(); } catch (error) { console.error("[Living Story] afterClose gagal", error); }
      }, 260);
    }
  }
  ...
  function init() {
    ...
    // Selalu tampil setiap kali dipanggil; isi pesan cuma berganti
    // ketika tanggalnya (WIB) sudah berbeda dari kunjungan terakhir.
    window.openLivingStoryNote = function (afterClose) {
      const info = getCycleInfo(getNow());
      const state = loadState();
      const message = getOrBuildMessage(info, state);

      window.__livingStoryAfterClose = typeof afterClose === "function" ? afterClose : null;
    ...
    window.getLivingStoryState = function () {
      ...
      return {
        ...info,
        ...state,
        message: state.lastShownDate === info.key && state.lastMessage
          ? state.lastMessage
          : getMessage(info, state),
        shouldShow: true
      };
    };
