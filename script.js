...
/* ---------- MOON PHASE + SHOOTING STAR WISH ---------- */
(function initNightSkyStory(){
  const moon = $("#moonPhase");
  const moonLabel = $("#moonPhaseLabel");
  ...
  function setMoonPhase(screenName){
    if (!moon) return;
    const idx = Math.max(0, SCREEN_ORDER.indexOf(screenName));
    const phase = idx % 8;
    const shifts = [-44,-25,-10,5,18,5,-10,-25];
    moon.style.setProperty("--moon-shift", `${shifts[phase]}px`);
    moon.dataset.phase = String(phase);
    if (moonLabel) moonLabel.textContent = phaseNames[phase];
  }
  ...
  window.addEventListener("screenchange", (e) => {
    setMoonPhase(e.detail);
    scheduleStar();
  });
  setMoonPhase(currentScreen || SCREEN_ORDER[0]);
  scheduleStar();
})();
