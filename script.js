...
const CONFIG = {
  ...
  letterEpilogue: [
    ...
    "Aku selalu mendoakanmu, di hari apa pun itu."
  ],
...
/* =========================================================
   PIN GATE — PIN selalu bisa diakses kapan saja.
   1 September hanya menentukan mode ulang tahun khusus.
   ========================================================= */
...
(function preloadAssets() {
  ["./img/hbd1.png?v=4", "./img/epilogue1.jpg?v=1", "img/polaroid-1.jpg", "img/polaroid-2.jpg", "img/polaroid-3.jpg", "img/polaroid-4.jpg"].forEach((src) => {
    const img = new Image();
    img.src = src;
  });
})();
...
const screens = {
  ...
  nope: $("#nope"), candle: $("#candle"), giftmap: $("#giftmap"), flower: $("#flower-screen"), farewell: $("#farewell"), final: $("#final"), reaction: $("#reaction"), reply: $("#reply"), behind: $("#behind"), endscreen: $("#endscreen")
};
const SCREEN_ORDER = ["pin", "playlist", "minigame", "intro", "letter", "wish", "mood", "spirit", "memories", "polaroid", "gift", "candle", "giftmap", "flower", "farewell", "final", "reaction", "reply", "behind", "endscreen"];
const music = $("#pin-music");
// Jangan loop satu lagu: event "ended" dipakai untuk pindah otomatis ke lagu berikutnya.
if (music) music.loop = false;
...
// Sinkronkan konten berbasis Plan A/B SEBELUM bouquet pernah dibuat terlihat.
// cycle-ready akan dinyalakan sendiri setelah asset bouquet yang benar siap.
document.body.classList.remove("cycle-ready");
applyLetterPhoto();
applyGiftmapCopy();
initDatePreview();
...
(function setupPhotoFallback() {
  ...
  if (!img) return;
  // Fallback lama sengaja dihapus: fallback ke hbd1.png bisa membuat
  // foto Plan A muncul ketika Plan B sedang aktif. applyLetterPhoto()
  // sekarang menangani error dengan aman (frame tetap tersembunyi).
...
function getPrettyScreenName(name) {
  return String(name)
    .replace("behind", "BEHIND THE SCENES")
    .replace("endscreen", "THE END")
    .replace("minigame", "MINI GAME")
    .replace("polaroid", "MEMORIES")
    .replace("farewell", "LAST LETTER")
    .replace("giftmap", isPlanBContent() ? "TITIP CERITA" : "GIFT FORM")
...
function showScreen(name) {
  ...
  if (name === "endscreen") startAfterglow();
}
function startAfterglow(){
  const el=$("#afterglow"); if(!el) return;
  el.classList.remove("show");
  void el.offsetWidth;
  setTimeout(()=>{
    if(currentScreen!=="endscreen") return;
    // Taruh heart-bloom & pesan penutup tepat di bawah signature (bukan
    // ditengah layar) supaya nggak pernah numpuk tombol/teks lain dan
    // signature tetap jadi elemen paling akhir, di semua ukuran layar.
    const sig = $(".end-signature");
    if (sig) el.style.setProperty("--sig-bottom", sig.getBoundingClientRect().bottom + "px");
    el.classList.add("show");
    setTimeout(()=>{ if(currentScreen === "endscreen") el.classList.remove("show"); }, 5600);
  }, 1200);
}
...
function selectSong(i, { forcePlay = false } = {}) {
  songManuallyChanged = true;
  const song = CONFIG.playlist[i];
  if (!song || !music) return;

  // Tap lagu yang sedang aktif = pause / play lagi. Jangan sentuh navigasi
  // layar dan jangan pernah reload dokumen.
  if (i === selectedSongIndex && music.currentSrc && music.currentSrc.endsWith(song.src.replace(/^\.\//, ""))) {
  ...
  selectedSongIndex = i;
  // PENTING: kalau dipanggil tepat setelah lagu SELESAI sendiri (event
  // "ended"), browser sudah otomatis men-set music.paused = true SEBELUM
  // baris ini jalan — jadi `!music.paused` bakal salah baca keadaan "lagu
  // baru aja habis, tapi playlist masih pengen lanjut" sebagai "user lagi
  // nge-pause". Makanya playNextSong() mengirim forcePlay:true supaya lagu
  // berikutnya TETAP diputar meskipun secara teknis music.paused true saat
  // titik ini. Ini yang bikin dulu lagu berhenti diam-diam & playlist gak
  // lanjut ke track selanjutnya begitu satu lagu selesai.
  const wasPlaying = forcePlay || !music.paused;
  const previousVolume = music.volume;
  const oldSrc = music.getAttribute("src") || "";
  const isNewSrc = oldSrc !== song.src;

  if (isNewSrc) {
    // GANTI TRACK PADA ELEMEN AUDIO YANG SAMA. Tidak memakai location,
    // showScreen(), atau reload apa pun — halaman/slide tetap persis di
    // tempatnya sekarang.
    music.pause();
    ...
    const playWhenReady = () => {
      music.removeEventListener("canplay", playWhenReady);
      if (wasPlaying || currentScreen === "playlist") {
        music.volume = previousVolume;
        startMusic();
      }
    };
    music.addEventListener("canplay", playWhenReady);
  } else if (wasPlaying) {
    startMusic();
  }
...
let isAdvancingSong = false;
async function playNextSong({wrap = true} = {}) {
  if (!CONFIG.playlist.length || isAdvancingSong) return;
  ...
  try {
    ...
    try {
      ...
      selectSong(nextIndex, { forcePlay: true });
      music.volume = 0;
      faded = true;
    ...
    // Selalu dilepas apa pun yang terjadi di atas (termasuk early-return
    // nextIndex<0) — sebelumnya flag ini bisa nyangkut "true" selamanya
    // kalau nextIndex<0, ikut mengunci SEMUA percobaan auto-next berikutnya.
    isAdvancingSong = false;
  }
}

// Pindah otomatis ketika lagu selesai; kembali ke lagu pertama setelah item terakhir.
...
/* ---------- SOLAR SYSTEM BACKDROP — delapan planet + Matahari.
   Setiap planet mengorbit Matahari dengan durasi berbeda dan berputar pada
   porosnya sendiri. Opacity sengaja rendah supaya tetap menjadi atmosfer,
   bukan mengganggu kartu cerita. ---------- */
const solarPlanets = ["mercury","venus","earth","mars","jupiter","saturn","uranus","neptune"];

/* ---------- INTERACTIVE SOLAR SYSTEM ---------- */
(function initPlanetExplorer(){
  const planets = document.querySelectorAll(".solar-planet[data-planet]");
  const info = $("#planetInfo"), name = $("#planetInfoName"), fact = $("#planetInfoFact");
  if (!planets.length || !info) return;
  planets.forEach((planet) => {
    planet.addEventListener("pointerdown", (e) => {
      e.stopPropagation();
      if (e.cancelable) e.preventDefault();
      if (name) name.textContent = planet.dataset.planet || "Planet";
      if (fact) fact.textContent = planet.dataset.fact || "a tiny world in our little universe.";
      ...
      info._hideTimer = setTimeout(() => { info.classList.remove("show"); info.setAttribute("aria-hidden","true"); }, 3600);
    }, {passive:false});
  });
})();

/* ---------- CONSTELLATION PROGRESS + SHOOTING STAR WISH ---------- */
(function initNightSkyStory(){
  const wrap = $("#constellationProgress");
  const label = $("#constellationLabel");
  const star = $("#shootingStar");
  const wish = $("#wishSpark");
  const TOTAL_STARS = 8;
  let starTimer = null;

  // Makin jauh progress cerita (dari SCREEN_ORDER), makin banyak bintang
  // rasi yang menyala & garisnya nyambung — pas sampai endscreen, rasinya
  // membentuk siluet hati yang utuh. Ganti dari konsep "moon phase" lama.
  function setConstellationProgress(screenName){
    if (!wrap) return;
    const idx = Math.max(0, SCREEN_ORDER.indexOf(screenName));
    const frac = SCREEN_ORDER.length > 1 ? idx / (SCREEN_ORDER.length - 1) : 1;
    const lit = Math.min(TOTAL_STARS, Math.max(1, Math.round(frac * (TOTAL_STARS - 1)) + 1));
    const complete = lit >= TOTAL_STARS;

    wrap.querySelectorAll(".const-star").forEach((s) => {
      s.classList.toggle("is-lit", Number(s.dataset.star) < lit);
    });
    wrap.querySelectorAll(".const-seg:not(.const-seg-close)").forEach((s) => {
      s.classList.toggle("is-drawn", Number(s.dataset.seg) < lit - 1);
    });
    const closeSeg = wrap.querySelector(".const-seg-close");
    if (closeSeg) closeSeg.classList.toggle("is-drawn", complete);
    wrap.classList.toggle("is-complete", complete);
    if (label) label.textContent = complete ? "rasi lengkap ✦" : `${lit}/${TOTAL_STARS} bintang`;
  }

  function launchShootingStar(){
    if (!star || currentScreen === "pin" || document.hidden) return;
    star.classList.remove("is-ready","is-flying");
    void star.offsetWidth;
    star.classList.add("is-flying");
    setTimeout(() => {
      if (!star) return;
      star.classList.remove("is-flying");
      if (currentScreen !== "pin") star.classList.add("is-ready");
    }, 2650);
  }

  function scheduleStar(){
    clearTimeout(starTimer);
    if (!star || currentScreen === "pin" || currentScreen === "playlist") return;
    starTimer = setTimeout(launchShootingStar, 5000 + Math.random()*5000);
  }
  ...
  star?.addEventListener("click", (e) => {
    ...
    try { localStorage.setItem("hbd-shooting-wish", "1"); } catch (_) {}
    playSfx("sparkle");
    vibrate([10,30,10]);
    showToast("Wish received ✦");
    setTimeout(scheduleStar, 3000);
  });

  window.addEventListener("screenchange", (e) => {
    setConstellationProgress(e.detail);
    scheduleStar();
  });
  setConstellationProgress(currentScreen || SCREEN_ORDER[0]);
  scheduleStar();
})();

/* ---------- animasi coding untuk slide 19 ---------- */
const BUILD_CODE = [
  '<span class="tok-tag">&lt;section</span> <span class="tok-attr">class</span>=<span class="tok-str">"little-something"</span><span class="tok-tag">&gt;</span>',
  '  <span class="tok-tag">&lt;h1&gt;</span><span class="tok-text">For you, with love.</span><span class="tok-tag">&lt;/h1&gt;</span>',
  '  <span class="tok-tag">&lt;button</span> <span class="tok-attr">id</span>=<span class="tok-str">"replay"</span><span class="tok-tag">&gt;</span>',
  '    <span class="tok-text">Replay ✦</span>',
  '  <span class="tok-tag">&lt;/button&gt;</span>',
  '<span class="tok-tag">&lt;/section&gt;</span>',
  '',
  '<span class="tok-comment">/* make every moment feel alive */</span>',
  '<span class="tok-css">.planet</span> { <span class="tok-attr">animation</span>: <span class="tok-str">orbit 18s linear infinite</span>; }',
  '<span class="tok-js">const story</span> = <span class="tok-str">"made for one person"</span>;',
  '<span class="tok-js">replay</span>.onclick = () =&gt; <span class="tok-fn">startFromBeginning</span>();'
];
const BUILD_TERMINAL = [
  ["npm run build", "compiling little memories..."],
  ["npm run polish", "adding stars, glow & tiny surprises..."],
  ["npm run test", "20 slides ✓  8 planets ✓  1 special person ✓"],
  ["npm run deploy", "build successful — made with love ♡"]
];
let codeTypingTimer = null;
let terminalTimer = null;
function startTerminalAnimation(){
  const cmd = $("#terminalTyping"), out = $("#terminalOutput");
  if (!cmd || !out) return;
  if (terminalTimer) clearTimeout(terminalTimer);
  let i = 0;
  const tick = () => {
    if (currentScreen !== "behind") return;
    const item = BUILD_TERMINAL[i % BUILD_TERMINAL.length];
    cmd.textContent = item[0]; out.textContent = item[1];
    i++; terminalTimer = setTimeout(tick, 1900);
  };
  tick();
}
function startCodeAnimation() {
  ...
  if (!el) return;
  if (codeTypingTimer) clearTimeout(codeTypingTimer);
  el.innerHTML = "";
  let line = 0, char = 0;
  const tick = () => {
    if (currentScreen !== "behind") return;
    if (line >= BUILD_CODE.length) {
      if (status) status.textContent = "built ✓";
      setTimeout(() => { if (currentScreen === "behind") startCodeAnimation(); }, 2600);
      return;
    }
    const raw = BUILD_CODE[line];
    // Type visible text while preserving the syntax markup once each line completes.
    if (char < raw.replace(/<[^>]*>/g, '').length) {
      const visible = raw.replace(/<[^>]*>/g, '');
      const targetText = visible.slice(0, char + 1);
      el.textContent = BUILD_CODE.slice(0, line).map(x => x.replace(/<[^>]*>/g,'')).join('\n') + (line ? '\n' : '') + targetText;
      char++;
      codeTypingTimer = setTimeout(tick, 16);
    } else {
      el.innerHTML = BUILD_CODE.slice(0, line + 1).join('\n');
      line++; char = 0;
      codeTypingTimer = setTimeout(tick, 130);
    }
  };
...
/* =========================================================
   COSMIC TOUCH + AMBIENT MOTION
   ========================================================= */
(function cosmicExperience(){
  const reduced = window.matchMedia && matchMedia("(prefers-reduced-motion: reduce)").matches;
  const glow = $("#cursorGlow");
  let gx=0, gy=0, raf=0;
  function moveGlow(x,y){
    if(!glow || reduced) return; gx=x; gy=y; if(raf) return;
    raf=requestAnimationFrame(()=>{ raf=0; glow.style.transform=`translate3d(${gx-130}px,${gy-130}px,0)`; glow.classList.add("visible"); });
  }
  document.addEventListener("pointermove",e=>moveGlow(e.clientX,e.clientY),{passive:true});
  function shootingStar(){
    if(reduced || document.hidden) return; const layer=$("#stars"); if(!layer) return;
    const s=document.createElement("i"); s.className="shooting-star cosmic-shoot";
    s.style.left=(8+Math.random()*72)+"%"; s.style.top=(5+Math.random()*45)+"%";
    s.style.setProperty("--shoot-angle",(-24-Math.random()*20)+"deg");
    s.style.setProperty("--shoot-x",(120+Math.random()*180)+"px");
    s.style.setProperty("--shoot-y",(80+Math.random()*130)+"px");
    layer.appendChild(s); setTimeout(()=>s.remove(),1900);
  }
  if(!reduced){ setTimeout(shootingStar,2400); setInterval(()=>{if(Math.random()<.72)shootingStar();},8500); }
  let lastX=null,lastY=null,lastZ=null,lastShake=0;
  function revealShakeSecret(){
    const hint=$("#shakeHint");
    if(hint){hint.textContent="♡ secret unlocked — you found the hidden little thing.";hint.classList.add("unlocked");}
    showToast("You found the secret ✦ I’m glad you’re here."); confetti(); vibrate([12,40,12]);
  }
  window.addEventListener("devicemotion",e=>{
    ...
    if(motionActivated && delta>24 && Date.now()-lastShake>1800){lastShake=Date.now();revealShakeSecret();}
  },{passive:true});
  const hint=$("#shakeHint");
  ...
  async function activateShake(){
    ...
    try{
      ...
      if(hint){ hint.textContent="✦ secret is listening… shake gently"; hint.classList.add("armed"); }
      showToast("Secret mode armed ✦");
      vibrate([8]);
    }catch(_){
      // Browser tanpa permission API tetap bisa mendengarkan devicemotion.
      motionActivated = true;
      if(hint){ hint.textContent="✦ secret is listening… shake gently"; hint.classList.add("armed"); }
    }
  }
  hint?.addEventListener("click",()=>{
    // First tap only arms the motion sensor; the secret stays hidden until a real shake.
    activateShake();
  });
})();
...
function applyLetterPhoto() {
  ...
  // IMPORTANT: jangan biarkan src bawaan HTML (Plan A) sempat terlihat
  // ketika halaman sedang berada di Plan B. Foto baru ditampilkan HANYA
  // setelah foto yang sesuai dengan plan selesai dimuat.
  imgEl.style.visibility = "hidden";
  imgEl.onload = () => {
    imgEl.style.visibility = "visible";
  };
  imgEl.onerror = () => {
    // Jangan pernah fallback Plan B -> foto Plan A. Kalau foto sesuai
    // plan gagal dimuat, lebih aman menyembunyikan frame daripada
    // menampilkan foto dari plan yang salah.
    imgEl.onload = null;
    imgEl.style.visibility = "hidden";
  };

  const isEpilogue = isPlanBContent();
  imgEl.src = isEpilogue
    ? "./img/epilogue1.jpg?v=1"
    : "./img/hbd1.png?v=4";
...
function openLetter() {
  playWipeTransition(() => {
    ...
    if (isEpilogue) {
      if (dateEl) dateEl.textContent = "hari apa pun itu ✦";
...
function goMinigame() { showScreen("minigame"); openMinigameMenu(); }
...
/* ---------- GIFTMAP COPY — dipanggil setiap kali layar giftmap
   ...
   pengunjung loncat langsung pakai navigasi).
   Plan A (dalam 30 hari menuju ulang tahun): form "pilih kado" persis
   seperti sebelumnya — peta rute, ship-tracker, grid pilihan kado.
   Plan B (di luar musim itu): konsepnya diganti total, bukan cuma
   ganti kata-kata — jadi layar "Titip Cerita", satu kotak curhat
   bebas/opsional, tanpa peta & tanpa grid pilihan kado sama sekali,
   karena hari biasa nggak butuh hadiah. ---------- */
function applyGiftmapCopy() {
  const isEpilogue = isPlanBContent();
  const eyebrowEl = $("#giftmapEyebrow");
  const headingEl = $("#giftmapHeading");
  const introEl = $("#giftmapIntro");
  const planAEl = $("#giftPlanA");
  const planBEl = $("#giftPlanB");
  if (isEpilogue) {
    if (eyebrowEl) eyebrowEl.textContent = "TITIP CERITA ✦";
    if (headingEl) headingEl.innerHTML = "Hari ini gimana?<br><em>cerita aja ke aku.</em>";
    if (introEl) introEl.textContent = "Keluh kesah, seneng-seneng, atau apapun itu — nggak wajib, tapi kalau mau, aku di sini dengerin ✦";
    if (planAEl) planAEl.style.display = "none";
    if (planBEl) planBEl.style.display = "";
  } else {
    ...
    if (planAEl) planAEl.style.display = "";
    if (planBEl) planBEl.style.display = "none";
  }
}
...
function applyFlowerCopy() {
  const isEpilogue = isPlanBContent();
  const bouquetScene = $("#bouquetScene");
  const sourceEl = $("#bouquetPicture source");
  const imgEl = $("#bouquetImage");
  // Saat mode berubah, sembunyikan dulu gambar lama. Baru tampilkan setelah
  // asset yang benar untuk Plan A/B sudah siap. Ini menghilangkan flash
  // bouquet ulang tahun ketika Plan B dibuka.
  document.body.classList.remove("cycle-ready");
  if (bouquetScene) bouquetScene.classList.add("mode-syncing");
  if (imgEl) imgEl.style.visibility = "hidden";
  const revealBouquet = () => {
    if (!imgEl) return;
    requestAnimationFrame(() => {
      imgEl.style.visibility = "visible";
      document.body.classList.add("cycle-ready");
      bouquetScene?.classList.remove("mode-syncing");
    });
  };
  ...
  if (isEpilogue) {
    ...
    if (imgEl) {
      imgEl.onerror = () => {
        // Never fall back to the Plan A birthday bouquet here. If the
        // Plan B asset is missing, keep the scene hidden rather than
        // flashing the birthday bouquet for a split second.
        imgEl.onerror = null;
        imgEl.onload = null;
        imgEl.style.visibility = "hidden";
        bouquetScene?.classList.remove("mode-syncing");
    ...
    if (imgEl) {
      imgEl.onerror = null;
      imgEl.onload = revealBouquet;
      imgEl.src = "./img/bouquet.png?v=1";
      imgEl.alt = "Sebuket bunga mawar pink dan ungu, dibungkus kertas dengan pita";
      if (imgEl.complete) revealBouquet();
    }
  }
}
...
function goBehind() { showScreen("behind"); startCodeAnimation(); startTerminalAnimation(); }
function goEndScreen() {
  document.body.classList.remove("celebrating", "final-cinematic");
...
function getBirthdayCycleState(now = getEffectiveNow()) {
  ...
  // 1 September adalah satu-satunya hari dengan mode ulang tahun.
  // Di luar itu, halaman tetap universal dan berjalan sebagai mode biasa.
  return (wibNow.month === 9 && wibNow.day === 1) ? "today" : "epilogue";
}
...
/* ---------- CYCLE THEME — satu tempat yang ngatur SEMUA teks/nuansa
   ...
   BERUBAH (diterapkan saat halaman dimuat atau tanggal simulasi berubah —
   jadi nggak ganggu fokus
   kalau pengunjung lagi ngetik PIN). ---------- */
function applyCycleTheme(state) {
  // Theme state controls styling/ambient effects only.
  // Keep the visible copy in index.html as the source of truth so
  // manual text edits are not overwritten by JavaScript on load.
  const body = document.body;

  body.classList.remove(
    "cycle-epilogue",
    "cycle-countdown-early",
    "cycle-countdown-close",
    "cycle-today"
  );
  body.classList.add("cycle-" + state);

  // Plan A dan Plan B punya kulit visual berbeda, tetapi tetap berbagi
  // struktur/isi experience yang hampir sama. Sinkronkan bouquet setiap kali
  // state tanggal berubah supaya tidak pernah membawa asset mode sebelumnya.
  if (typeof applyFlowerCopy === "function") applyFlowerCopy();

  // Fireworks can still react to the current date cycle without
  // replacing any text that was intentionally edited in index.html.
  if (state === "epilogue") {
    fireworksMode = "off";
  } else if (state === "countdown-close" || state === "today") {
    fireworksMode = "hype";
  } else {
...
/* ---------- CATATAN TANGGAL YANG BISA DITAP — tampil sebagai teks info
   ...
   dateOverride diisi dan mode ulang tahun/non-ulang tahun ikut diperbarui
   tanpa reload.
...
function refreshCycleTheme() {
  const state = getBirthdayCycleState();
...
$("#endReplayBtn").onclick = () => {
  playSfx("tap");
  vibrate(12);
  document.body.classList.remove("celebrating", "final-cinematic");
  $(".end-actions")?.removeAttribute("hidden");
  // Replay benar-benar dari awal: PIN kembali menjadi gerbang pertama dan
  // progress lokal dibersihkan agar pengalaman tidak melompat ke halaman lama.
  maxReachedIndex = 0;
  isTargetUser = false;
...
$("#endCloseBtn").onclick = () => {
  ...
  // Browser hanya mengizinkan window.close() untuk tab/jendela yang
  // memang dibuka oleh script. Untuk GitHub Pages yang dibuka langsung,
  // fallback-nya adalah mengganti halaman dengan about:blank — jadi tidak
  // muter balik ke layar "closed" yang sama.
  try {
    window.open("", "_self");
    window.close();
  } catch (_) {}
...
const giftObject = $("#giftObject");
giftObject?.classList.add("gift-idle");
giftObject?.addEventListener("click", () => {
  const b = $("#giftObject");
  if (!giftOpened) {
    giftOpened = true;
    giftObject?.classList.remove("gift-idle");
    playSfx("whoosh");
...
$("#behindNextBtn").onclick = () => { playSfx("tap"); goEndScreen(); };
...
$("#giftOptions")?.addEventListener("click", (e) => {
  ...
  // .gift-option pakai overflow:hidden (biar ripple-nya rapi), jadi sparkle
  // ditaruh di container gridnya (.gift-options, overflow:visible) supaya
  // ledakan bintangnya nggak kepotong sama tepi tombol.
  spawnSparkleBurst($("#giftOptions"), e.clientX, e.clientY);
});
...
/* ---------- Plan B — Titip Cerita: satu kotak curhat bebas & opsional,
   dikirim lewat jalur yang sama (Formspree/WA fallback) kayak gift
   request, tapi boleh dilewati kalau kosong ---------- */
async function sendDiaryEntry(entry) {
  if (!CONFIG.wishFormEndpoint) return false;
  try {
    const response = await fetch(CONFIG.wishFormEndpoint, {
      method: "POST",
...
$("#diarySendBtn")?.addEventListener("click", async () => {
  const status = $("#diaryStatus");
  const entry = $("#diaryEntryInput")?.value.trim() || "";
  if (!entry) {
    playSfx("tap");
    if (status) status.textContent = "kosong nih — tulis dikit aja, atau tap \"Lewati aja\" ✦";
    return;
  }
  playSfx("tap");
  if (status) status.textContent = "mengirim...";
  const ok = await sendDiaryEntry(entry);
  if (ok) {
    if (status) status.textContent = "ceritamu udah nyampe ke aku ♡";
  } else if (CONFIG.replyWhatsapp) {
    const msg = encodeURIComponent(`Titip cerita dari ${CONFIG.name}:\n${entry}`);
    window.open(`https://wa.me/${CONFIG.replyWhatsapp}?text=${msg}`, "_blank");
    if (status) status.textContent = "kebuka di WhatsApp — tinggal tap kirim ya ✦";
  } else {
    if (status) status.textContent = "belum ada tujuan pengiriman diatur di situs ini ✦";
...
$("#diarySkipBtn")?.addEventListener("click", () => {
  playSfx("tap");
  vibrate(8);
  applyFlowerCopy();
  showScreen("flower");
...
const STEP_GOTO = {
  ...
  reaction: goReaction,
  reply: goReply,
  behind: () => showScreen("behind"),
  endscreen: () => showScreen("endscreen")
};
...
if ("serviceWorker" in navigator && !IS_LOCAL_DEV) {
  window.addEventListener("load", () => {
    ...
    navigator.serviceWorker.addEventListener("controllerchange", () => {
      // Jangan paksa reload halaman. Pengunjung bisa tetap membaca cerita
      // dan musik tidak ikut terputus hanya karena SW mendapat versi baru.
      if (hadControllerAtLoad) showToast("Versi baru siap — tetap di halaman ini ✦");
    });
  });
}
...
/* =========================================================
   ...
   ========================================================= */
(function () {
  const RIPPLE_SELECTOR = ".primary-btn,.ghost-btn,.icon-btn,.round-btn,.mood-btn,.playlist-item,.reaction-btn,.wish-pill,.polaroid,.flock-btn,.gift-option";
...
/* =========================================================
   PLAN A / PLAN B — PREMIUM INTERACTIVE UPGRADE
   1) Tilt-parallax generik untuk elemen ".tilt-card" (dipakai di
      map-scene Plan A & diary-scene Plan B): saat disentuh/di-hover,
      kartu miring 3D mengikuti posisi jari/kursor, dan tiap child
      ber-atribut [data-depth] ikut bergeser sesuai "kedalaman"-nya
      sendiri sehingga terasa berlapis (parallax), bukan cuma miring
      datar. Otomatis nonaktif kalau prefers-reduced-motion.
   2) spawnSparkleBurst() — ledakan bintang kecil di titik tap,
      dipakai saat memilih kado & saat pena diketuk.
   3) Ship-tracker: tap salah satu step memunculkan tooltip kecil
      berisi cerita singkat soal tahap itu (murni dekoratif).
   4) Pena "Titip Cerita": tap ikon pena → animasi tanda tangan +
      sparkle, plus idle wobble halus supaya terasa hidup.
   Semua bagian ini murni visual/interaktif tambahan — tidak
   menyentuh logic pengiriman gift-request/diary yang sudah ada.
   ========================================================= */
(function () {
  ...
  function initTiltCard(card) {
    ...
    function apply(clientX, clientY) {
      ...
      card.style.transform = `perspective(760px) rotateX(${(-py * 12).toFixed(2)}deg) rotateY(${(px * 13).toFixed(2)}deg)`;
      layers.forEach((l) => {
        const depth = parseFloat(l.dataset.depth) || 0;
        const base = l.dataset.baseTransform || "";
        l.style.transform = `${base} translate3d(${(px * depth).toFixed(1)}px, ${(py * depth * 0.7).toFixed(1)}px, 0)`;
      });
    }
    function reset() {
      card.classList.remove("is-tilting");
      card.style.transform = "";
      layers.forEach((l) => { l.style.transform = l.dataset.baseTransform || ""; });
    }
    function start(e) {
  ...
  window.spawnSparkleBurst = function (anchorEl, clientX, clientY, count) {
    ...
    const posParent = getComputedStyle(anchorEl).position;
    if (posParent === "static") anchorEl.style.position = "relative";
    anchorEl.appendChild(wrap);
    setTimeout(() => wrap.remove(), 850);
  };

  /* ---------- 3) ship-tracker tooltip ---------- */
  document.querySelectorAll(".ship-step[data-tip]").forEach((step) => {
    let tip = null;
    let hideTimer = null;
    function openTip() {
      clearTimeout(hideTimer);
      document.querySelectorAll(".ship-step.tip-open").forEach((s) => { if (s !== step) closeTip(s); });
      if (!tip) {
        tip = document.createElement("span");
        tip.className = "ship-step-tip";
        tip.textContent = step.dataset.tip;
        step.appendChild(tip);
      }
      requestAnimationFrame(() => step.classList.add("tip-open"));
      hideTimer = setTimeout(() => closeTip(step), 2600);
    }
    function closeTip(el) {
      (el || step).classList.remove("tip-open");
    }
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest(".ship-step")) {
      document.querySelectorAll(".ship-step.tip-open").forEach((s) => s.classList.remove("tip-open"));
    }
  });

  /* ---------- 4) pena "Titip Cerita" — tanda tangan + sparkle ---------- */
  const diaryIcon = document.getElementById("diaryIcon");
  if (diaryIcon) {
    diaryIcon.addEventListener("click", (e) => {
      diaryIcon.classList.remove("is-signing");
      void diaryIcon.offsetWidth;
      diaryIcon.classList.add("is-signing");
      if (typeof playSfx === "function") playSfx("tap");
      if (typeof vibrate === "function") vibrate(8);

      const scene = diaryIcon.closest(".diary-scene");
      if (scene) {
        const old = scene.querySelector(".diary-signature-trail");
        if (old) old.remove();
        const trail = document.createElement("span");
        trail.className = "diary-signature-trail";
        trail.innerHTML =
          '<svg viewBox="0 0 120 40" width="120" height="40" aria-hidden="true">' +
          '<path d="M6,26 C18,6 26,6 34,20 C40,30 46,10 56,14 C66,18 70,32 82,20 C90,12 96,24 114,14"></path>' +
          "</svg>";
        scene.appendChild(trail);
        setTimeout(() => trail.remove(), 750);
      }
      // .diary-scene pakai overflow:hidden (biar glow/stars-nya rapi di
      // dalam kartu), jadi sparkle ditaruh di parent-nya (#giftPlanB,
      // overflow:visible) supaya ledakannya nggak kepotong.
      spawnSparkleBurst(diaryIcon.closest("#giftPlanB") || scene, e.clientX, e.clientY, 6);
    });
  }
})();
...
/* =========================================================
   MINI GAME HUB + CONSTELLATION CONNECT
   ========================================================= */
const constellation = {
  points: [], next: 0, running: false, won: false, timer: 30, interval: null,
  ctx: null, dpr: 1, width: 0, height: 0, raf: null
};
function setMinigamePanel(id) {
  ["minigameMenu","memoryGamePanel","constellationGamePanel"].forEach(pid => {
    const el = document.getElementById(pid);
    if (el) el.classList.toggle("active", pid === id);
  });
}
function openMinigameMenu(){
  setMinigamePanel("minigameMenu");
  constellation.running = false;
  clearInterval(constellation.interval);
  cancelAnimationFrame(constellation.raf);
}
function openMemoryGame(){
  setMinigamePanel("memoryGamePanel");
  buildMemoryGrid();
  playSfx("tap");
}
function openConstellationGame(){
  setMinigamePanel("constellationGamePanel");
  resetConstellation();
  playSfx("tap");
}
...
function resizeConstellationCanvas(){
  ...
  constellation.dpr = Math.min(window.devicePixelRatio || 1, 2);
  constellation.width = Math.max(280, rect.width);
  constellation.height = Math.max(300, rect.height);
  canvas.width = Math.round(constellation.width * constellation.dpr);
  canvas.height = Math.round(constellation.height * constellation.dpr);
  constellation.ctx = canvas.getContext("2d");
  constellation.ctx.setTransform(constellation.dpr,0,0,constellation.dpr,0,0);
  drawConstellation();
}
function makeConstellationPoints(){
  const w=constellation.width||320,h=constellation.height||320;
  const raw=[[.18,.25],[.42,.12],[.68,.20],[.82,.42],[.64,.67],[.39,.82],[.16,.65]];
  constellation.points=raw.map(([x,y],i)=>({x:w*x,y:h*y,r:12,label:i+1,pulse:Math.random()*Math.PI*2}));
}
function resetConstellation(){
  clearInterval(constellation.interval); cancelAnimationFrame(constellation.raf);
  constellation.running=false; constellation.won=false; constellation.next=0; constellation.timer=30;
  const start=document.getElementById("constellationStartBtn"), next=document.getElementById("constellationNextBtn");
  if(start){start.style.display="inline-flex";start.disabled=false;start.querySelector("span").textContent="Mulai";}
  if(next){next.style.display="none";next.disabled=true;}
  const status=document.getElementById("constellationStatus"); if(status) status.textContent="Temukan bintang pertama ✦";
  const prog=document.getElementById("constellationProgress"); if(prog) prog.textContent="1 / 7";
  const timer=document.getElementById("constellationTimer"); if(timer) timer.textContent="30s";
  resizeConstellationCanvas(); makeConstellationPoints(); drawConstellation();
}
function startConstellation(){
  if(constellation.running) return;
  constellation.running=true; constellation.won=false; constellation.next=0; constellation.timer=30;
  const start=document.getElementById("constellationStartBtn"); if(start) start.style.display="none";
  const status=document.getElementById("constellationStatus"); if(status) status.textContent="Cari bintang nomor 1 ✦";
  clearInterval(constellation.interval);
  constellation.interval=setInterval(()=>{
    constellation.timer--; const t=document.getElementById("constellationTimer"); if(t)t.textContent=constellation.timer+"s";
    if(constellation.timer<=0) failConstellation("Waktunya habis… coba lagi ✦");
  },1000);
  drawConstellation(); playSfx("tap"); vibrate(8);
}
function failConstellation(message){
  constellation.running=false; clearInterval(constellation.interval);
  const status=document.getElementById("constellationStatus"); if(status) status.textContent=message;
  const start=document.getElementById("constellationStartBtn"); if(start){start.style.display="inline-flex";start.querySelector("span").textContent="Coba lagi";}
  drawConstellation();
}
function winConstellation(){
  constellation.running=false; constellation.won=true; clearInterval(constellation.interval);
  const status=document.getElementById("constellationStatus"); if(status) status.textContent="Konstelasinya ketemu ✦ ternyata kamu memang jago.";
  const next=document.getElementById("constellationNextBtn"); if(next){next.style.display="inline-flex";next.disabled=false;}
  playSfx("chime"); vibrate([20,40,35]); burstMiniConfetti(next||document.getElementById("constellationCanvas"));
  drawConstellation();
}
function constellationPointer(e){
  if(!constellation.running || constellation.won) return;
  const canvas=document.getElementById("constellationCanvas"); if(!canvas)return;
  const rect=canvas.getBoundingClientRect();
  const x=e.clientX-rect.left,y=e.clientY-rect.top;
  const p=constellation.points[constellation.next]; if(!p)return;
  if(Math.hypot(x-p.x,y-p.y)<=Math.max(22,p.r+10)){
    p.hit=true; constellation.next++;
    const prog=document.getElementById("constellationProgress"); if(prog)prog.textContent=Math.min(constellation.next+1,7)+" / 7";
    const status=document.getElementById("constellationStatus"); if(status)status.textContent=constellation.next>=7?"Semua bintang tersambung ✦":"Bagus! Cari bintang nomor "+(constellation.next+1)+" ✦";
    playSfx("tap"); vibrate(6);
    if(constellation.next>=7) winConstellation(); else drawConstellation();
  } else {
    const status=document.getElementById("constellationStatus"); if(status)status.textContent="Ups, itu bukan bintang berikutnya ✦";
    vibrate(18); drawConstellation();
  }
}
function drawConstellation(){
  const ctx=constellation.ctx, w=constellation.width,h=constellation.height;
  if(!ctx||!w)return;
  ctx.clearRect(0,0,w,h);
  // subtle starfield
  for(let i=0;i<45;i++){
    const x=(i*83.7)%w,y=(i*137.3)%h,r=(i%3===0)?1.4:.7;
    ctx.globalAlpha=.18+(i%4)*.07; ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fillStyle="#fff";ctx.fill();
  }
  ctx.globalAlpha=1;
  for(let i=0;i<constellation.points.length-1;i++){
    if(!constellation.points[i].hit || !constellation.points[i+1].hit) break;
    const a=constellation.points[i],b=constellation.points[i+1];
    ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.lineWidth=2;ctx.strokeStyle="rgba(220,205,255,.82)";ctx.shadowBlur=12;ctx.shadowColor="rgba(180,140,255,.9)";ctx.stroke();ctx.shadowBlur=0;
  }
  const now=performance.now()/500;
  constellation.points.forEach((p,i)=>{
    const pulse=1+Math.sin(now+p.pulse)*.08, active=i===constellation.next&&constellation.running;
    ctx.beginPath();ctx.arc(p.x,p.y,(active?18:13)*pulse,0,Math.PI*2);ctx.fillStyle=active?"rgba(255,230,160,.16)":"rgba(200,180,255,.09)";ctx.fill();
    ctx.beginPath();ctx.arc(p.x,p.y,active?6:4.5,0,Math.PI*2);ctx.fillStyle="#fff";ctx.shadowBlur=active?22:12;ctx.shadowColor=active?"#ffe3a3":"#cdb8ff";ctx.fill();ctx.shadowBlur=0;
    ctx.font="600 10px system-ui";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillStyle="rgba(255,255,255,.9)";ctx.fillText(String(i+1),p.x,p.y-25);
  });
  if(constellation.running){ constellation.raf=requestAnimationFrame(drawConstellation); }
}

const memoryChoice=document.getElementById("memoryChoice"); if(memoryChoice) memoryChoice.onclick=openMemoryGame;
const constellationChoice=document.getElementById("constellationChoice"); if(constellationChoice) constellationChoice.onclick=openConstellationGame;
const memoryBack=document.getElementById("memoryBackBtn"); if(memoryBack) memoryBack.onclick=()=>{playSfx("tap");openMinigameMenu();};
const constellationBack=document.getElementById("constellationBackBtn"); if(constellationBack) constellationBack.onclick=()=>{playSfx("tap");openMinigameMenu();};
const constellationStart=document.getElementById("constellationStartBtn"); if(constellationStart) constellationStart.onclick=startConstellation;
const constellationNext=document.getElementById("constellationNextBtn"); if(constellationNext) constellationNext.onclick=()=>{playSfx("tap");goIntro();};
const constellationCanvas=document.getElementById("constellationCanvas");
if(constellationCanvas){
  constellationCanvas.addEventListener("pointerdown",constellationPointer);
  window.addEventListener("resize",()=>{ if(document.getElementById("constellationGamePanel")?.classList.contains("active")){resizeConstellationCanvas();makeConstellationPoints();drawConstellation();} });
}
