...
/* =========================================================
   MINI GAME — A BOUQUET FOR YOU
   ========================================================= */
const BOUQUET_FLOWERS = {
  rose:{emoji:'🌹', name:'Rose', color:'rose'},
  tulip:{emoji:'🌷', name:'Tulip', color:'tulip'},
  sunflower:{emoji:'🌻', name:'Sunflower', color:'sunflower'},
  blossom:{emoji:'🌸', name:'Blossom', color:'blossom'},
  baby:{emoji:'🤍', name:"Baby's Breath", color:'baby'}
};
let bouquetChosen=[];
function resetBouquetGame(){
  bouquetChosen=[];
  const heads=$('#gameFlowerHeads'); if(heads) heads.innerHTML='';
  $$('.flower-pick').forEach(b=>{b.classList.remove('picked','disabled')});
  const progress=$('#bouquetProgress'); if(progress) progress.textContent='0 / 4 bunga';
  const status=$('#bouquetStatus'); if(status) status.textContent='pilih bunga pertama ✦';
  const next=$('#bouquetNextBtn'); if(next) next.disabled=true;
  const clue=$('#bouquetClue'); if(clue) clue.textContent='“Aku ingin memberikan sesuatu yang berarti...”';
}
function addBouquetFlower(key, btn){
  if(bouquetChosen.length>=4 || bouquetChosen.includes(key)) return;
  bouquetChosen.push(key);
  btn.classList.add('picked');
  const f=BOUQUET_FLOWERS[key], heads=$('#gameFlowerHeads');
  if(heads){
    const el=document.createElement('span'); el.className='game-flower '+f.color; el.textContent=f.emoji; el.style.setProperty('--i', bouquetChosen.length); heads.appendChild(el);
  }
  playSfx('chime');
  $('#bouquetProgress').textContent=bouquetChosen.length+' / 4 bunga';
  const messages=['Satu untuk rasa sayang.','Satu untuk senyum yang selalu diingat.','Satu untuk semua doa baik.','Dan satu… khusus untuk kamu.'];
  $('#bouquetStatus').textContent=messages[bouquetChosen.length-1];
  if(bouquetChosen.length===4){
    $$('.flower-pick:not(.picked)').forEach(b=>b.classList.add('disabled'));
    $('#bouquetClue').textContent='“Kayaknya… bouquet ini memang dibuat buat kamu.”';
    $('#bouquetNextBtn').disabled=false;
    burstMiniConfetti($('#bouquetNextBtn'));
  }
}
function goBouquet(){
  showScreen('minigame');
  $('#minigameMenu').classList.remove('active');
  $('#memoryGamePanel').classList.remove('active');
  $('#bouquetGamePanel').classList.add('active');
  resetBouquetGame();
}
function openMinigameMenu(){
  $('#minigameMenu').classList.add('active');
  $('#memoryGamePanel').classList.remove('active');
  $('#bouquetGamePanel').classList.remove('active');
}
function initBouquetGame(){
  $$('.flower-pick').forEach(btn=>btn.addEventListener('click',()=>addBouquetFlower(btn.dataset.flower,btn)));
  $('#bouquetChoice')?.addEventListener('click',goBouquet);
  $('#memoryChoice')?.addEventListener('click',()=>{ $('#minigameMenu').classList.remove('active'); $('#bouquetGamePanel').classList.remove('active'); $('#memoryGamePanel').classList.add('active'); buildMemoryGrid(); });
  $('#memoryBackBtn')?.addEventListener('click',openMinigameMenu);
  $('#bouquetBackBtn')?.addEventListener('click',openMinigameMenu);
  $('#bouquetNextBtn')?.addEventListener('click',()=>{playSfx('chime'); burstMiniConfetti($('#bouquetNextBtn')); setTimeout(goIntro,520);});
}
initBouquetGame();
