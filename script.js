...
/* =========================================================
   MINI GAME HUB — Memory Match + Constellation Connect
   ========================================================= */
...
function setMiniGamePanel(name) {
  ["minigameMenu", "memoryGamePanel", "constellationGamePanel"].forEach(id => {
    const el = $("#" + id);
    if (el) el.classList.toggle("active", id === name);
  });
  if (name === "constellationGamePanel") resetConstellation();
}
...
const CONSTELLATION_POINTS = [
  {x:.18,y:.72},{x:.30,y:.50},{x:.43,y:.30},{x:.58,y:.42},{x:.74,y:.20},{x:.82,y:.48},{x:.67,y:.76}
];
let constellationState = { started:false, next:0, mistakes:0, timer:30, interval:null, points:[], dpr:1 };
function constellationCanvasSize() {
  const c = $("#constellationCanvas");
  if (!c) return;
  const rect = c.getBoundingClientRect();
  constellationState.dpr = Math.min(window.devicePixelRatio || 1, 2);
  c.width = Math.max(1, Math.floor(rect.width * constellationState.dpr));
  c.height = Math.max(1, Math.floor(rect.height * constellationState.dpr));
  const ctx = c.getContext("2d");
  ctx.setTransform(constellationState.dpr,0,0,constellationState.dpr,0,0);
  constellationState.points = CONSTELLATION_POINTS.map(p => ({x:p.x*rect.width, y:p.y*rect.height}));
  drawConstellation();
}
function drawConstellation() {
  ...
  // soft space dust
  for (let i=0;i<45;i++) {
    const x=(i*83)%Math.max(1,rect.width), y=(i*47)%Math.max(1,rect.height);
    ctx.globalAlpha=.18 + ((i%4)*.06); ctx.fillStyle="#fff"; ctx.beginPath(); ctx.arc(x,y,(i%3)+.5,0,Math.PI*2); ctx.fill();
  }
  ctx.globalAlpha=1;
  const pts=constellationState.points;
  ctx.lineCap="round";
  for(let i=0;i<Math.min(constellationState.next-1,pts.length-1);i++){
    ctx.strokeStyle="rgba(255,220,245,.9)"; ctx.lineWidth=2.5; ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[i+1].x,pts[i+1].y); ctx.stroke();
  }
  pts.forEach((p,i)=>{
    const active=i===constellationState.next && constellationState.started;
    const done=i<constellationState.next;
    ctx.beginPath(); ctx.arc(p.x,p.y,active?10:done?6:5,0,Math.PI*2);
    ctx.fillStyle=done?"rgba(255,230,248,.95)":"rgba(255,255,255,.88)"; ctx.shadowColor="rgba(255,170,225,.8)"; ctx.shadowBlur=active?18:8; ctx.fill(); ctx.shadowBlur=0;
    ctx.font="600 10px system-ui"; ctx.fillStyle="rgba(255,255,255,.72)"; ctx.textAlign="center"; ctx.fillText(String(i+1),p.x,p.y-14);
  });
}
function resetConstellation() {
  clearInterval(constellationState.interval);
  constellationState={started:false,next:0,mistakes:0,timer:30,points:[],dpr:1};
  const status=$("#constellationStatus"), start=$("#constellationStartBtn"), next=$("#constellationNextBtn");
  if(status) status.textContent="Temukan bintang pertama ✦";
  if(start){start.style.display="inline-flex";start.disabled=false;start.querySelector("span").textContent="Mulai";}
  if(next) next.disabled=true;
  $("#constellationProgress").textContent="1 / 7";
  $("#constellationTimer").textContent="30s";
  requestAnimationFrame(constellationCanvasSize);
}
function startConstellation() {
  constellationState.started=true; constellationState.next=0; constellationState.mistakes=0; constellationState.timer=30;
  $("#constellationStartBtn").style.display="none";
  $("#constellationStatus").textContent="Sentuh bintang nomor 1 ✦";
  constellationCanvasSize();
  clearInterval(constellationState.interval);
  constellationState.interval=setInterval(()=>{
    constellationState.timer--;
    $("#constellationTimer").textContent=constellationState.timer+"s";
    if(constellationState.timer<=0) finishConstellation(false);
  },1000);
  drawConstellation();
}
function finishConstellation(won) {
  clearInterval(constellationState.interval); constellationState.started=false;
  if(won){
    $("#constellationStatus").textContent="Kamu berhasil membentuk konstelasinya ✦";
    $("#constellationNextBtn").disabled=false; playSfx("chime"); burstMiniConfetti($("#constellationNextBtn"));
  }else{
    $("#constellationStatus").textContent="Waktunya habis ✦ coba sekali lagi";
    $("#constellationStartBtn").style.display="inline-flex"; $("#constellationStartBtn").querySelector("span").textContent="Coba lagi";
  }
  drawConstellation();
}
function handleConstellationPointer(e) {
  if(!constellationState.started) return;
  const c=$("#constellationCanvas"), r=c.getBoundingClientRect();
  const x=e.clientX-r.left, y=e.clientY-r.top;
  const target=constellationState.points[constellationState.next];
  if(!target) return;
  if(Math.hypot(x-target.x,y-target.y)<26){
    constellationState.next++;
    $("#constellationProgress").textContent=Math.min(constellationState.next+1,7)+" / 7";
    playSfx("tap");
    if(constellationState.next>=constellationState.points.length) finishConstellation(true);
    else $("#constellationStatus").textContent="Bagus ✦ sekarang bintang "+(constellationState.next+1);
    drawConstellation();
  }else{
    constellationState.mistakes++;
    c.animate([{transform:"translateX(-3px)"},{transform:"translateX(3px)"},{transform:"translateX(0)"}],{duration:180});
    $("#constellationStatus").textContent="Ups, bukan itu ✦ cari bintang "+(constellationState.next+1);
  }
}

$("#memoryChoice").onclick=()=>{playSfx("tap");setMiniGamePanel("memoryGamePanel");buildMemoryGrid();};
$("#constellationChoice").onclick=()=>{playSfx("tap");setMiniGamePanel("constellationGamePanel");};
$("#memoryBackBtn").onclick=()=>{playSfx("tap");setMiniGamePanel("minigameMenu");};
$("#constellationBackBtn").onclick=()=>{playSfx("tap");setMiniGamePanel("minigameMenu");};
$("#constellationStartBtn").onclick=()=>{playSfx("tap");startConstellation();};
$("#constellationNextBtn").onclick=()=>{playSfx("tap");goIntro();};
$("#constellationCanvas").addEventListener("pointerdown",handleConstellationPointer);
window.addEventListener("resize",()=>{if($("#constellationGamePanel")?.classList.contains("active")) constellationCanvasSize();});
...
function goMinigame() { showScreen("minigame"); setMiniGamePanel("minigameMenu"); }
