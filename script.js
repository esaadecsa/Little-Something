...
function isSeptemberFirstWIB(date = new Date()) {
...
const CONFIG = {
  ...
  pin: "0101",
  // Playlist ala Spotify di layar pertama. Item paling atas (index 0) =
  ...
  // jadi lagu TIDAK PERNAH restart/putus waktu pindah slide atau balik
  // ke slide sebelumnya). Urutan Untouchable/Panjang Umur otomatis
  // ketuker tergantung IS_BIRTHDAY_TODAY (siapa pun yang "lagi jadwalnya"
  // taruh di index 0 biar itu yang autoplay) — tapi DUA-duanya selalu ada
  // di daftar, jadi pengunjung tetap bebas pilih manual kapan pun, 6 lagu total.
  playlist: (IS_BIRTHDAY_TODAY
    ? [
...
setInterval(() => { if (Math.random() < .72) spawnAsteroid(); }, 2800);
...
setInterval(() => { if (Math.random() < .55) spawnSatellite(); }, 9000);
...
setInterval(() => { if (Math.random() < .4) spawnAsteroidCollision(); }, 15000);
setTimeout(spawnAsteroidCollision, 6000); // satu kejadian cepat di awal biar langsung kerasa
...
setInterval(() => { if (Math.random() < .35) spawnMeteorShower(); }, 24000);
...
/* ---------- MICRO-TRANSITIONS — wipe/curtain & page-turn, dipakai di
   momen-momen tertentu (bukan semua slide) supaya berasa lebih sinematik
   dibanding transisi fade standar. Keduanya "melewatkan" callback saja
   kalau overlay tidak ada atau prefers-reduced-motion aktif. ---------- */
function playWipeTransition(callback) {
  ...
  setTimeout(() => overlay.classList.remove("wipe-play"), 900);
}
function playPageTurn(callback) {
  const overlay = $("#pageTurnOverlay");
  if (!overlay || window.matchMedia("(prefers-reduced-motion: reduce)").matches) { callback(); return; }
  overlay.classList.add("pt-play");
  setTimeout(callback, 480);
  setTimeout(() => overlay.classList.remove("pt-play"), 970);
}
...
function primaryAction() {
  ...
  if (currentScreen === "giftmap") return showScreen("flower");
...
/* =========================================================
   SCREEN FLOW
   ========================================================= */
function openLetter() { playPageTurn(() => { startMusic(); showScreen("letter"); typeWriter(CONFIG.letter); }); }
...
$("#pinSubmitBtn")?.addEventListener("click", (e) => {
  ...
  if (val === CONFIG.pin) {
    ...
    playSfx("chime");
    vibrate(16);
    playAccessGranted(() => showScreen("playlist"));
  } else {
    playSfx("tap");
...
function goCandle() {
  playWipeTransition(() => goCandleInner());
}
...
function finishCelebration() {
  ...
  setTimeout(() => { document.body.classList.remove("candle-climax"); showScreen("giftmap"); playSfx("chime"); }, 1550);
}
...
$("#giftmapNextBtn").onclick = async () => {
  ...
  playSfx("chime");
  vibrate(16);
  setTimeout(() => showScreen("flower"), 900);
};
