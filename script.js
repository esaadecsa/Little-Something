...
const screens = {
  ...
  nope: $("#nope"), candle: $("#candle"), giftmap: $("#giftmap"), flower: $("#flower-screen"), farewell: $("#farewell"), final: $("#final"), reaction: $("#reaction"), reply: $("#reply"), keepsake: $("#keepsake"), endscreen: $("#endscreen")
};
const SCREEN_ORDER = ["pin", "playlist", "minigame", "intro", "letter", "wish", "mood", "spirit", "memories", "polaroid", "gift", "candle", "giftmap", "flower", "farewell", "final", "reaction", "reply", "keepsake", "endscreen"];
...
function getPrettyScreenName(name) {
  return String(name)
    .replace("keepsake", "A LITTLE KEEPSAKE")
...
/* =========================================================
   MINI GAME — MEMORY MATCH + A BOUQUET FOR YOU
   ========================================================= */
...
function buildMemoryGrid() {
  ...
  grid.innerHTML = ""; memoryFirst = null; memoryLock = false; memoryMatches = 0;
  $("#minigameStatus").textContent = "0 / 4 pasang ditemukan"; $("#minigameNextBtn").disabled = true;
  shuffle([...MEMORY_PHOTOS, ...MEMORY_PHOTOS]).forEach(src => {
    const el = document.createElement("button"); el.type="button"; el.className="memory-card"; el.dataset.src=src;
    el.innerHTML = `<div class="memory-card-inner"><div class="memory-face back"><span>✦</span><small>♡</small></div><div class="memory-face front"><img src="${src}" alt="Kenangan" loading="lazy"></div></div>`;
    el.addEventListener("click", () => onMemoryCardClick(el)); grid.appendChild(el);
  });
}
function onMemoryCardClick(el) {
  if (memoryLock || el.classList.contains("flipped") || el.classList.contains("matched")) return;
  el.classList.add("flipped"); playSfx("tap");
  if (!memoryFirst) { memoryFirst = el; return; }
  memoryLock = true; const a=memoryFirst,b=el; memoryFirst=null;
  if (a.dataset.src===b.dataset.src) {
    setTimeout(()=>{ a.classList.add("matched"); b.classList.add("matched"); memoryMatches++; $("#minigameStatus").textContent=memoryMatches+" / 4 pasang ditemukan"; playSfx("chime"); memoryLock=false; if(memoryMatches===4){ $("#minigameStatus").textContent="semua kenangan berhasil ditemukan ✦"; $("#minigameNextBtn").disabled=false; burstMiniConfetti($("#minigameNextBtn")); } },360);
  } else setTimeout(()=>{a.classList.remove("flipped");b.classList.remove("flipped");memoryLock=false;},720);
}

const BOUQUET_FLOWERS = {
  rose:{label:"Rose",mark:"✿",tone:"rose"}, tulip:{label:"Tulip",mark:"♢",tone:"tulip"}, sunflower:{label:"Sunflower",mark:"✺",tone:"sunflower"}, blossom:{label:"Blossom",mark:"✿",tone:"blossom"}, baby:{label:"Baby's Breath",mark:"·",tone:"baby"}
};
let bouquetChosen=[];
function resetBouquetGame(){
  bouquetChosen=[];
  $("#gameFlowerHeads") && ($("#gameFlowerHeads").innerHTML="");
  $("#bouquetProgress") && ($("#bouquetProgress").textContent="0 / 4");
  $("#bouquetStatus") && ($("#bouquetStatus").textContent='pilih bunga yang terasa paling “kamu” ✦');
  $("#bouquetNextBtn") && ($("#bouquetNextBtn").disabled=true);
  document.querySelectorAll(".flower-pick").forEach(b=>b.classList.remove("chosen"));
  document.querySelectorAll("#bouquetSlots i").forEach(i=>{i.className="";i.textContent="✿";});
  $("#bouquetStage")?.classList.remove("complete");
}
function addBouquetFlower(key, btn){
  if(bouquetChosen.length>=4 || bouquetChosen.includes(key)) return;
  bouquetChosen.push(key); btn.classList.add("chosen"); playSfx("chime");
  const f=BOUQUET_FLOWERS[key], head=document.createElement("span"); head.className=`bouquet-mini-flower ${f.tone}`; head.innerHTML=`<b>${f.mark}</b>`; head.title=f.label;
  $("#gameFlowerHeads").appendChild(head);
  head.style.setProperty("--x", `${(Math.random()*48)-24}px`); head.style.setProperty("--r", `${(Math.random()*16)-8}deg`);
  const slot=$("#bouquetSlots").querySelectorAll("i")[bouquetChosen.length-1]; if(slot){slot.className=`filled ${f.tone}`;slot.textContent=f.mark;}
  $("#bouquetProgress").textContent=`${bouquetChosen.length} / 4`;
  $("#bouquetStatus").textContent = bouquetChosen.length===4 ? "your bouquet is ready ♡" : "one more little flower... ✦";
  if(bouquetChosen.length===4){ $("#bouquetNextBtn").disabled=false; $("#bouquetStage").classList.add("complete"); burstMiniConfetti($("#bouquetNextBtn")); }
}
function openMinigameMenu(){ $("#minigameMenu")?.classList.add("active"); $("#memoryGamePanel")?.classList.remove("active"); }
function showMiniPanel(id){ document.querySelectorAll("#minigame .minigame-panel").forEach(p=>p.classList.remove("active")); $(id)?.classList.add("active"); }
$("#memoryChoice")?.addEventListener("click",()=>{playSfx("tap");showMiniPanel("#memoryGamePanel");buildMemoryGrid();});
$("#memoryBackBtn")?.addEventListener("click",()=>{playSfx("tap");goPlaylist();});
document.querySelectorAll(".flower-pick").forEach(btn=>btn.addEventListener("click",()=>addBouquetFlower(btn.dataset.flower,btn)));
$("#minigameNextBtn")?.addEventListener("click",()=>{playSfx("tap");goIntro();});
$("#bouquetNextBtn")?.addEventListener("click",()=>{playSfx("tap");goIntro();});
...
function goMinigame() {
  showScreen("minigame");
  // Memory Match is the single mini-game: open it immediately.
  $("#minigameMenu")?.classList.remove("active");
  $("#memoryGamePanel")?.classList.add("active");
  buildMemoryGrid();
}
...
function goKeepsake() { showScreen("keepsake"); }
...
function handlePinSubmit(e) {
  ...
  // Normalisasi hanya spasi di ujung. PIN tetap harus persis 0101.
  const val = String(input.value || "").trim();
  const error = $("#pinError");

  if (!val) {
    error && (error.textContent = "Masukkan PIN dulu ✦");
    input.focus();
    return;
  }
  ...
  if (val === String(CONFIG.pin)) {
    ...
    try {
      ...
      if (Number.isFinite(saved) && saved > 0) {
        resumedIdx = Math.min(saved, SCREEN_ORDER.length - 1);
      }
    } catch (_) {}
    ...
    const continueAfterNote = () => {
      showScreen("playlist");
      if (resumedIdx > 1) showToast("lanjut dari terakhir kali ya ✦");
    };

    playAccessGranted(() => {
      // Living Story adalah tambahan, bukan syarat login. Kalau engine
      // tidak tersedia / error, PIN tetap harus bisa membuka cerita.
      let shown = false;
      try {
        shown = typeof window.openLivingStoryNote === "function"
          ? window.openLivingStoryNote(continueAfterNote) === true
          : false;
      } catch (_) {
        shown = false;
      }
      if (!shown) continueAfterNote();
    });
  } else {
    playSfx("tap");
    if (error) error.textContent = "PIN salah, coba lagi ✦";
    input.classList.remove("shaking");
    // Paksa reflow supaya animasi shake tetap jalan kalau salah berkali-kali.
...
const keepsakeNotes = document.querySelectorAll(".keepsake-note");
keepsakeNotes.forEach(note => note.addEventListener("click", () => {
  note.classList.toggle("opened");
  playSfx("chime");
  if ([...keepsakeNotes].every(n => n.classList.contains("opened"))) {
    $("#keepsakeNextBtn").disabled = false;
    $(".keepsake-hint")?.replaceChildren(document.createTextNode("three little reminders, just for you ♡"));
    burstMiniConfetti($("#keepsakeNextBtn"));
  }
}));

$("#keepsakeNextBtn").onclick = () => { playSfx("tap"); goEndScreen(); };
...
/* =========================================================
   LIVING STORY — ONE YEAR OF LITTLE ENCOURAGEMENTS
   The site keeps a gentle companion note for a full yearly cycle.
   It is date-based (WIB), not refresh-based. September 1 is always
   the birthday reset: the annual cycle starts again and the special
   birthday greeting returns.
   ========================================================= */

/* =============================================================
   COSMIC MICRO-WORLDS — subtle moving objects on every page.
   Uses one lightweight fixed layer so Plan A/B and all story screens
   share the same little-universe atmosphere.
   ============================================================= */
(function initCosmicMicroWorlds(){
  if (document.querySelector('.cosmic-particles')) return;
  const layer = document.createElement('div');
  layer.className = 'cosmic-particles';
  ...
  const palettes = [
    ['#aaa1ff','#5b4ca9','#17142f','rgba(125,112,255,.42)'],
    ['#e9b8ff','#814db1','#241433','rgba(213,151,255,.34)'],
    ['#b9d8ff','#4769a5','#111b31','rgba(116,174,255,.32)'],
    ['#ffd5ea','#a65382','#271020','rgba(255,133,192,.28)']
  ];

  for(let i=0;i<count;i++){
    const el=document.createElement('i');
    const r=Math.random();
    const type=r<.48?'is-star':r<.74?'is-moon':'is-planet';
    el.className='cosmic-particle '+type;
    if(type==='is-planet' && Math.random()<.24) el.classList.add('is-ring');
    const size=type==='is-star' ? (1.5+Math.random()*2.1) : type==='is-moon' ? (4+Math.random()*5) : (7+Math.random()*12);
    const palette=palettes[Math.floor(Math.random()*palettes.length)];
    el.style.left=(5+Math.random()*90).toFixed(2)+'%';
    el.style.top=(5+Math.random()*90).toFixed(2)+'%';
    el.style.setProperty('--cp-size',size.toFixed(1)+'px');
    el.style.setProperty('--cp-duration',(8+Math.random()*10).toFixed(2)+'s');
    el.style.setProperty('--cp-delay',(-Math.random()*12).toFixed(2)+'s');
    el.style.setProperty('--cp-x',(-24+Math.random()*48).toFixed(1)+'px');
    el.style.setProperty('--cp-y',(-26+Math.random()*52).toFixed(1)+'px');
    if(type==='is-planet'){
      el.style.setProperty('--cp-light',palette[0]);
      el.style.setProperty('--cp-main',palette[1]);
      el.style.setProperty('--cp-dark',palette[2]);
      el.style.setProperty('--cp-glow',palette[3]);
    }
    layer.appendChild(el);
  }
})();
