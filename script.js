...
(function cosmicExperience(){
  ...
  window.addEventListener("devicemotion",e=>{
    ...
    if(delta>24 && Date.now()-lastShake>1800){lastShake=Date.now();revealShakeSecret();}
  },{passive:true});
  const hint=$("#shakeHint");
  hint?.addEventListener("click",()=>revealShakeSecret());
  hint?.addEventListener("click",async()=>{try{if(typeof DeviceMotionEvent!=="undefined"&&typeof DeviceMotionEvent.requestPermission==="function")await DeviceMotionEvent.requestPermission();}catch(_){}},{once:true});
})();
