...
setInterval(() => { if (!window.EDITORIAL_THEME && isTabActive() && Math.random() < .45) spawnShootingStar(); }, 8000);

function spawnMeteorShower() {
  const n = 3 + Math.floor(Math.random() * 3);
  for (let i = 0; i < n; i++) setTimeout(spawnShootingStar, i * (160 + Math.random() * 120));
}
setInterval(() => { if (!window.EDITORIAL_THEME && isTabActive() && Math.random() < .25) spawnMeteorShower(); }, 55000);
...
setInterval(() => { if (!window.EDITORIAL_THEME && isTabActive() && Math.random() < .45) spawnMeteor(); }, 18000);
...
(function cosmicExperience(){
  ...
  if(!reduced && !window.EDITORIAL_THEME){ setTimeout(shootingStar,2400); setInterval(()=>{if(Math.random()<.72)shootingStar();},8500); }
...
$("#pinSubmitBtn")?.addEventListener("click", (e) => {
  ...
  if (val === CONFIG.pin) {
    ...
    playAccessGranted(() => {
      ...
      // Living Story benar-benar menjadi bagian dari alur masuk:
      // PIN benar → akses dibuka → catatan hari ini → baru masuk Playlist.
      const shown = typeof window.openLivingStoryNote === "function"
        ? window.openLivingStoryNote(continueAfterNote)
        : false;

      // Kalau catatan hari ini sudah pernah dibaca, langsung lanjut.
